...
{
    Matrix<T> t_mat(this->height, this->width);

    for (int i=0 ; i<width ; i++)
    {
        for (int j=0 ; j<height ; j++)
        {
            t_mat.Mat[i][j] = this->Mat[j][i];
        }
    }
    return t_mat;
}
...
template <class T>
void Matrix<T>::addMat(Matrix<T>const& mat1,Matrix<T>& mat2)
{
    if(mat1.height != this->height && mat1.width != this->width)
    {
        std::cerr << "Matrix dimension should be same" << std::endl;
    }
    else
    {
        for(int i = 0; i < this->height; i++)
        {
            ...
            {
                mat2.Mat[i][j] = this->Mat[i][j] + mat1.Mat[i][j];
            }
        }
    }
}
...
template <class T>
void Matrix<T>::pow(Matrix<T>& mat1, uint16_t num)
{
    if(mat1.height != this->height && mat1.width != this->width)
    {
        std::cerr << "Matrix dimension should be same" << std::endl;
    ...
    else
    {
        for(int i = 0; i < this->height; i++)
        {
            for(int j = 0; j < this->width; j++)
            {
                mat1.Mat[i][j] = this->Mat[i][j]*this->Mat[i][j];
            }
        }
    }
}
...
template <class T>
void Matrix<T>::sqrt_mat(Matrix<T>& mat1)
{
    if(mat1.height != this->height && mat1.width != this->width)
    {
        std::cerr << "Matrix dimension should be same" << std::endl;
